/****************************************************************************
 * arch/risc-v/src/hpm5300/hpm_lowputc.c
 *
...
int hpm_uart_configure(uint32_t base, const struct uart_config_s *config)
{
  ...
  /* Set DLAB to 1 (the SDK uart_init does a simple read-modify-write) */

  lcr = getreg32(base + HPM_UART_LCR_OFFSET);
  putreg32(lcr | UART_LCR_DLAB, base + HPM_UART_LCR_OFFSET);
  ...
  /* Reset the TX/RX FIFOs and enable them (the SDK uart_init does the
   * same at the end of its configuration sequence).  Trigger level 0
   * means the RX interrupt fires as soon as one byte is available.
   */

  putreg32(UART_FCRR_TFIFORST | UART_FCRR_RFIFORST,
           base + HPM_UART_FCRR_OFFSET);
  putreg32(UART_FCRR_FIFOE | UART_FCRR_FIFOT4EN |
           UART_FCRR_RFIFOT4(0) | UART_FCRR_TFIFOT4(0),
           base + HPM_UART_FCRR_OFFSET);

  /* Enable the UART receive input.  The RXEN bit resets to 0, which
   * holds the RX input high; it must be set after the pin mux is
   * configured (the SDK uart_init does the same), otherwise no data
   * ever reaches the RX FIFO.
   */

  modifyreg32(base + HPM_UART_IDLE_CFG_OFFSET, 0, UART_IDLE_CFG_RXEN);
